...
from page_cache import PageImageCache
...
# from realtime_cleanup import cleanup_current_page_images
from image_cleanup import cleanup_images_on_exit
import zipfile, datetime, base64
from image_cleanup import cleanup_images_on_exit
# from realtime_cleanup import cleanup_current_page_images
from image_trash_manager import cleanup_book_images, cleanup_orphaned_images, cleanup_page_images, cleanup_all_images_on_backup_restore
    ...
    def dragEnterEvent(self, event):
        ...
        if event.mimeData().hasUrls():
            ...
            if urls:  # 하나 이상의 파일 허용
                ...
                if valid_images > 0:  # 하나 이상의 유효한 이미지가 있으면 허용
                ...
                if parent:
                    ...
                    if valid_images:
                        if len(valid_images) == 1:
                            # 단일 파일은 기존 방식 사용
                            parent.load_image_from_path(valid_images[0])
                        else:
                            # 여러 파일은 load_multiple_images 사용
                            parent.load_multiple_images(valid_images)
...
class PromptBook(QMainWindow):
    ...
    def __init__(self):
        ...
        # 페이지 이미지 캐시 초기화
        self.page_cache = PageImageCache(max_size=10)
    ...
    def setup_menubar(self):
        ...
        self.custom_transparency_level = 1.0  # 0.0 (완전 투명) ~ 1.0 (완전 불투명) - 기본 100% 불투명도
    ...
    def setup_image_view(self):
        ...
        self.thumbnail_bar.image_selected.connect(self.on_thumbnail_selected)
        self.thumbnail_bar.image_added.connect(self.on_thumbnail_image_added)
        self.thumbnail_bar.image_removed.connect(self.on_thumbnail_image_removed)
        self.thumbnail_bar.images_updated.connect(self.on_thumbnail_images_updated)
        self.right_layout.addWidget(self.thumbnail_bar)
    ...
    def clear_image_view(self):
        """이미지 뷰어 클리어"""
        try:
            if hasattr(self, 'image_view') and hasattr(self, 'image_scene'):
                # 씬에서 모든 아이템 제거
                self.image_scene.clear()
                
                # 현재 테마에 맞는 배경색 설정
                if hasattr(self, 'current_theme') and hasattr(self, 'THEMES'):
                ...
                # 드롭 힌트 가시성 업데이트
                if hasattr(self.image_view, 'update_drop_hint_visibility'):
                    self.image_view.update_drop_hint_visibility()
                
                print("[DEBUG] 이미지 뷰어 클리어 완료")
        except Exception as e:
            print(f"[ERROR] 이미지 뷰어 클리어 실패: {e}")
    ...
    def on_character_reordered(self):
        ...
        print("[DEBUG] 새 페이지 저장 중...")
        self.save_to_file()
        print("[DEBUG] 새 페이지 저장 완료")
    ...
    def save_ui_settings(self):
        settings = {
            ...
            "custom_transparency_level": getattr(self, "custom_transparency_level", 1.0),
    ...
    def load_ui_settings_early(self):
        ...
        try:
            with open(self.SETTINGS_FILE, 'r', encoding='utf-8') as f:
                ...
                # 커스텀 투명도 설정 복원
                self.custom_transparency_level = settings.get("custom_transparency_level", 1.0)
    ...
    def on_book_selected(self, index):
        ...
        if len(selected_books) > 1:
            ...
            if hasattr(self, 'thumbnail_bar'):
                self.thumbnail_bar.clear_all_images()
    ...
    def save_to_file(self):
        ...
        print(f"[DEBUG] 저장 시작: {self.SAVE_FILE}")
        print(f"[DEBUG] _initial_loading: {getattr(self, '_initial_loading', 'None')}")
        print(f"[DEBUG] _toggling_favorite: {getattr(self, '_toggling_favorite', 'None')}")
        
        if getattr(self, '_initial_loading', False):
            print("[DEBUG] 저장 건너뜀: _initial_loading=True")
            return
        
        # 즐겨찾기 토글 중일 때는 저장하지 않음 (이벤트 충돌 방지)
        if getattr(self, '_toggling_favorite', False):
            print("[DEBUG] 저장 건너뜀: _toggling_favorite=True")
            return
        
        try:
            print(f"[DEBUG] 저장할 북 수: {len(self.state.books)}")
            for book_name, book_data in self.state.books.items():
                pages_count = len(book_data.get('pages', []))
                print(f"[DEBUG]   {book_name}: {pages_count}개 페이지")
            
            with open(self.SAVE_FILE, 'w', encoding='utf-8') as f:
                json.dump(self.state.books, f, ensure_ascii=False, indent=2)
                print(f"[DEBUG] JSON 저장 성공!")
                print(f"[DEBUG] 저장 완료: {len(self.state.books)}개 북")
    ...
    def load_from_file(self):
        if os.path.exists(self.SAVE_FILE):
            try:
                ...
                # 정상 로드 완료 후 초기 로딩 플래그 해제
                self._initial_loading = False
                print("[DEBUG] 초기 로딩 완료, 저장 기능 활성화")
            ...
            # 파일이 없는 경우도 초기 로딩 플래그 해제
            self._initial_loading = False
            print("[DEBUG] 저장 파일 없음, 초기 로딩 플래그 해제")
    ...
    def load_preview_image(self):
        ...
        current_page_name = self.state.characters[self.current_index].get("name", "페이지")
        added_images = []
        
        print(f"[DEBUG] 선택된 이미지 파일 {len(file_paths)}개 처리 시작")
        
        # 모든 선택된 이미지를 썸네일바에 추가
        for file_path in file_paths:
            ...
            try:
                if hasattr(self, 'thumbnail_bar') and self.thumbnail_bar:
                    # 썸네일바의 파일명 규칙에 따라 복사하여 추가
                    new_path = self.thumbnail_bar.copy_image_with_naming_rule(file_path, current_page_name)
                    if new_path:
                        self.thumbnail_bar.add_image(new_path)
                        added_images.append(new_path)
                        print(f"[DEBUG] 규칙에 따라 이미지 복사 및 추가: {os.path.basename(new_path)}")
                    else:
                        # 복사 실패 시 원본 경로 사용
                        self.thumbnail_bar.add_image(file_path)
                        added_images.append(file_path)
                        print(f"[DEBUG] 원본 경로로 이미지 추가: {os.path.basename(file_path)}")
                else:
                    print("[WARN] 썸네일바가 없습니다.")
                    
            except Exception as e:
                print(f"[ERROR] 이미지 추가 실패 {os.path.basename(file_path)}: {e}")
        
        # 첫 번째 이미지가 추가되었으면 자동 선택
        if added_images and hasattr(self, 'thumbnail_bar') and self.thumbnail_bar:
            self.thumbnail_bar.select_image(added_images[0])
            print(f"[DEBUG] 첫 번째 이미지 자동 선택: {os.path.basename(added_images[0])}")
        ...
        print(f"[SUCCESS] 이미지 불러오기 완료: {len(added_images)}개 추가됨")
    def load_image_from_path(self, file_path):
        """파일 경로로부터 이미지를 로드하는 공통 메서드 - 썸네일바 시스템 사용"""
        if not file_path or not os.path.exists(file_path):
        ...
        try:
            if hasattr(self, 'thumbnail_bar') and self.thumbnail_bar:
                ...
                if new_path:
                    self.thumbnail_bar.add_image(new_path)
                    # 추가된 이미지를 자동 선택
                    self.thumbnail_bar.select_image(new_path)
                    print(f"[DEBUG] 규칙에 따라 이미지 복사 및 추가: {os.path.basename(new_path)}")
                else:
                    # 복사 실패 시 원본 경로 사용
                    self.thumbnail_bar.add_image(file_path)
                    self.thumbnail_bar.select_image(file_path)
                    print(f"[DEBUG] 원본 경로로 이미지 추가: {os.path.basename(file_path)}")
            else:
                print("[WARN] 썸네일바가 없습니다.")
                return
        ...
        print(f"[SUCCESS] 이미지 로드 완료 (썸네일바 시스템)")
        
    def load_multiple_images(self, file_paths):
        """여러 이미지 파일을 로드하는 메서드 - 뷰포트 드래그 앤 드롭용"""
        if not file_paths:
            return
        ...
        print(f"[DEBUG] 뷰포트 드래그 앤 드롭: {len(file_paths)}개 이미지 처리 시작")
        ...
        print(f"[SUCCESS] 뷰포트 드래그 앤 드롭 완료: {len(added_images)}개 추가됨")
        ...
        if len(selected_pages) > 1:
            ...
            for i, char in enumerate(self.state.characters):
                if char.get("name") == name:
                    ...
                    # 썸네일바에 페이지 이미지들 자동 로드 (캐시 활용)
                    if hasattr(self, 'thumbnail_bar'):
                        # 현재 페이지 이름 설정
                        page_name = char.get("name")
                        self.thumbnail_bar.set_current_page_name(page_name)
                        
                        # 캐시에서 먼저 확인
                        print(f"[DEBUG] 캐시 확인 중 - 페이지: {page_name}, 북: {self.current_book}")
                        print(f"[DEBUG] page_cache 객체: {type(self.page_cache)}")
                        cached_images = self.page_cache.get(self.current_book or "default", page_name)
                        
                        if cached_images is not None:
                            # 캐시 히트 - 바로 로드 (빠른 로딩 모드)
                            self.thumbnail_bar.load_page_images(cached_images, fast_load=True)
                        else:
                            # 캐시 미스 - 데이터에서 로드 후 캐시에 저장
                            page_images = self.thumbnail_bar.get_page_images_from_character(char)
                            self.thumbnail_bar.load_page_images(page_images, fast_load=False)
                            if page_images:
                                self.page_cache.put(self.current_book or "default", page_name, page_images)
                    
                    # 페이지 변경 시 실시간 이미지 정리
                    # cleanup_current_page_images(self)
    ...
    def update_image_buttons_state(self):
        ...
        # 이미지 제거 버튼: 페이지가 선택되어 있고 이미지가 있을 때 활성화
        has_image = False
        current_image_path = ""
        
        if page_selected:
            # 메인 이미지 경로 확인
            main_image_path = self.state.characters[self.current_index].get("image_path", "")
            has_main_image = bool(main_image_path and os.path.exists(main_image_path))
            
            # 썸네일바에서 현재 선택된 이미지 확인
            thumbnail_image = None
            if hasattr(self, 'thumbnail_bar') and self.thumbnail_bar:
                thumbnail_image = self.thumbnail_bar.get_current_image()
            
            # 썸네일바에 이미지가 있거나 메인 이미지가 있으면 제거 버튼 활성화
            has_thumbnail_image = bool(thumbnail_image and os.path.exists(thumbnail_image))
            has_image = has_main_image or has_thumbnail_image
            
            current_image_path = thumbnail_image if has_thumbnail_image else main_image_path
        
        self.image_remove_btn.setEnabled(has_image)
        
        # EXIF 체크박스: 이미지가 있을 때만 활성화
        self.exif_checkbox.setEnabled(has_image)
        print(f"[DEBUG] 이미지 버튼 상태 업데이트: 메인={bool(main_image_path if page_selected else False)}, "
              f"썸네일={bool(thumbnail_image if page_selected else False)}, "
              f"제거버튼={has_image}, 현재이미지={os.path.basename(current_image_path) if current_image_path else '없음'}")
    ...
    def remove_preview_image(self):
        """현재 선택된 이미지를 제거 (썸네일바 우선)"""
        if 0 <= self.current_index < len(self.state.characters):
            # 1. 썸네일바에서 현재 선택된 이미지 먼저 확인
            current_thumbnail = None
            if hasattr(self, 'thumbnail_bar') and self.thumbnail_bar:
                current_thumbnail = self.thumbnail_bar.get_current_image()
            
            # 2. 제거할 이미지 경로 결정
            image_to_remove = None
            if current_thumbnail and os.path.exists(current_thumbnail):
                # 썸네일바에서 선택된 이미지가 있으면 우선 제거
                image_to_remove = current_thumbnail
                print(f"[DEBUG] 썸네일바에서 선택된 이미지 제거: {os.path.basename(image_to_remove)}")
            else:
                # 썸네일바에 선택된 이미지가 없으면 메인 이미지 제거
                main_image_path = self.state.characters[self.current_index].get("image_path", "")
                if main_image_path and os.path.exists(main_image_path):
                    image_to_remove = main_image_path
                    print(f"[DEBUG] 메인 이미지 제거: {os.path.basename(image_to_remove)}")
            
            if not image_to_remove:
                print("[DEBUG] 제거할 이미지가 없습니다.")
                return
            
            # 3. 파일을 휴지통으로 이동
            try:
                if hasattr(self, 'send2trash') and self.send2trash:
                    self.send2trash(image_to_remove)
                    print(f"[DEBUG] 이미지 파일을 휴지통으로 이동: {image_to_remove}")
                else:
                    # send2trash가 없으면 일반 삭제
                    os.remove(image_to_remove)
                    print(f"[DEBUG] 이미지 파일 삭제: {image_to_remove}")
            except Exception as e:
                print(f"[ERROR] 이미지 파일 삭제 실패: {e}")
            
            # 4. 페이지 데이터에서 이미지 경로 제거
            char = self.state.characters[self.current_index]
            
            # 메인 이미지 경로인 경우 제거
            if char.get("image_path") == image_to_remove:
                char["image_path"] = ""
                print("[DEBUG] 메인 이미지 경로 클리어")
            
            # additional_images에서도 제거
            if "additional_images" in char and image_to_remove in char["additional_images"]:
                char["additional_images"].remove(image_to_remove)
                print("[DEBUG] 추가 이미지 목록에서 제거")
            
            # 5. 썸네일바에서 제거 (아직 제거되지 않은 경우)
            if hasattr(self, 'thumbnail_bar') and self.thumbnail_bar:
                if image_to_remove in self.thumbnail_bar.get_all_images():
                    self.thumbnail_bar.remove_image(image_to_remove)
                    print(f"[DEBUG] 썸네일바에서 위젯 제거: {os.path.basename(image_to_remove)}")
            
            # 6. 이미지 뷰어 클리어 (현재 표시된 이미지가 제거된 경우)
            try:
                # 현재 표시된 이미지와 제거된 이미지가 같으면 뷰어 클리어
                current_view_empty = True
                if hasattr(self, 'thumbnail_bar') and self.thumbnail_bar:
                    remaining_images = self.thumbnail_bar.get_all_images()
                    if remaining_images:
                        # 남은 이미지가 있으면 첫 번째 이미지 표시
                        self.thumbnail_bar.select_image(remaining_images[0])
                        current_view_empty = False
                
                if current_view_empty:
                    # 남은 이미지가 없으면 뷰어 클리어
                    self.image_scene.clear()
                    self.image_view.update_drop_hint_visibility()
                    print("[DEBUG] 이미지 뷰어 클리어")
                
            except Exception as e:
                print(f"[ERROR] 이미지 뷰어 업데이트 실패: {e}")
            ...
            print(f"[SUCCESS] 이미지 제거 완료: {os.path.basename(image_to_remove)}")
        ...
        if reply == QMessageBox.Yes:
            ...
            for i, char in enumerate(self.state.characters):
                if char.get("name") in page_names:
                    ...
                    # 페이지의 모든 이미지를 휴지통으로 이동
                    cleanup_page_images(char)
    ...
    def delete_selected_character(self):
        ...
        if reply == QMessageBox.Yes:
            # 페이지의 모든 이미지들을 휴지통으로 이동 (메인 + 추가 이미지들)
            page_data = self.state.characters[self.current_index]
            cleanup_page_images(page_data)
    ...
    def rename_character_dialog(self, item):
        ...
        if ok and new_name and new_name != old_name:
            ...
            # 썸네일바에서 이미지 파일명 자동 변경
            if hasattr(self, 'thumbnail_bar'):
                renamed_files = self.thumbnail_bar.rename_page_images(old_name, new_name)
                if renamed_files:
                    print(f"[DEBUG] 페이지 '{old_name}' → '{new_name}' 변경으로 {len(renamed_files)}개 이미지 파일명 변경됨")

                    # 변경된 파일 경로를 캐릭터 데이터에 업데이트
                    for char in self.state.characters:
                        if char["name"] == new_name:
                            if "image_path" in char:
                                old_path = char["image_path"]
                                for old_file_path, new_file_path in renamed_files:
                                    if old_path == old_file_path:
                                        char["image_path"] = new_file_path
                                        print(f"[DEBUG] 캐릭터 데이터의 이미지 경로 업데이트: {new_file_path}")
                                        break
                            break
    ...
    def apply_theme(self, theme_name):
        ...
        # 썸네일바 테마 적용
        if hasattr(self, 'thumbnail_bar') and hasattr(self.thumbnail_bar, 'apply_theme'):
            self.thumbnail_bar.apply_theme(theme)
            print(f"[INFO] 썸네일바에 테마 적용: {theme_name}")
    ...
    def adjust_custom_theme_transparency(self):
        ...
        try:
            ...
            # 현재 투명도 표시
            # 사용자 설정 투명도 레벨 사용 (기본값을 1.0으로 변경)
            transparency = getattr(self, 'custom_transparency_level', 1.0)
            
            # =========================
            # 기본 100% 불투명도 + 조절바 값으로만 제어
            # ========================= 
            transparency_label = QLabel(f"UI 투명도: {int(transparency * 100)}%")
            ...
            transparency_slider.setMaximum(100)  # 최대 100% (완전 불투명도)
            transparency_slider.setValue(int(transparency * 100))
            ...
            medium_button.clicked.connect(lambda: (transparency_slider.setValue(70), brightness_slider.setValue(50)))
            ...
            reset_button.clicked.connect(lambda: (transparency_slider.setValue(100), brightness_slider.setValue(50)))
            button_layout.addWidget(reset_button)
    ...
    def apply_custom_theme_transparency_new(self):
        ...
        try:
            ...
            # 검색창들 - 조절바 값 그대로 (기본 100% 불투명도)
            # 검색창들 - 조절바 값 그대로 (보정 제거됨)
            search_style = f"""
            ...
            # 텍스트 입력 - 조절바 값 그대로 (기본 100% 불투명도)
            # 텍스트 입력 - 조절바 값 그대로 (보정 제거됨)
            text_style = f"""
                background-color: rgba({self.hex_to_rgba(theme['surface'])}, {transparency});
                border: 1px solid {theme['border']};
                color: {theme['text']};
            ...
            # 버튼들 - 조절바 값 그대로 (기본 100% 불투명도)
            # 버튼들 - 조절바 값 그대로 (보정 제거됨)
            button_hover_transparency = min(transparency + 0.15, 0.95)  # hover 시만 약간 더 진하게  # hover 시 더 진하게
            button_style = f"""
                QPushButton {{
            ...
            # 썸네일바 투명도 적용
            if hasattr(self, 'thumbnail_bar') and hasattr(self.thumbnail_bar, 'apply_custom_transparency'):
                current_theme = self.THEMES.get(self.current_theme, self.THEMES['어두운 모드'])
                self.thumbnail_bar.apply_custom_transparency(current_theme, self)
                print(f"[INFO] 썸네일바에 커스텀 투명도 적용: {transparency}")

            print(f"[DEBUG] UI 투명도 단순화 완료: 기본=100%, 조절바={transparency:.2f}")
            print(f"[DEBUG] 모든 UI 요소가 동일한 투명도로 통일됨")
...
<li>🔒 아이콘으로 잠금 상태 확인</li>
    ...
    def on_thumbnail_selected(self, image_path):
        """썸네일 선택 시 메인 뷰포트에 이미지 표시"""
        if image_path and os.path.exists(image_path):
            self.update_image_view(image_path)
            # 썸네일 선택 시 제거 버튼 활성화
            self.image_remove_btn.setEnabled(True)
            self.exif_checkbox.setEnabled(True)
            print(f"[DEBUG] 썸네일에서 이미지 선택: {os.path.basename(image_path)}")
        elif not image_path:
            # 빈 경로인 경우 이미지 뷰어 클리어
            ...
            self.image_remove_btn.setEnabled(False)
            self.exif_checkbox.setEnabled(False)
            print("[DEBUG] 모든 썸네일 제거됨 - 이미지 뷰어 클리어")
        else:
            print(f"[ERROR] 썸네일 이미지 파일이 존재하지 않습니다: {image_path}")
            self.clear_image_view()
            # 오류 시 제거 버튼 비활성화
            self.image_remove_btn.setEnabled(False)
            self.exif_checkbox.setEnabled(False)
    ...
    def on_thumbnail_image_added(self, image_path):
        """썸네일바에 이미지 추가 시 페이지 데이터 업데이트"""
        if 0 <= self.current_index < len(self.state.characters):
            char = self.state.characters[self.current_index]
            
            # additional_images 필드가 없으면 생성
            if "additional_images" not in char:
                char["additional_images"] = []
            
            # 중복 방지
            if image_path not in char["additional_images"]:
                char["additional_images"].append(image_path)
                print(f"[DEBUG] 페이지 데이터에 이미지 추가: {os.path.basename(image_path)}")
                
                # 캐시 업데이트
                if hasattr(self, 'page_cache') and 0 <= self.current_index < len(self.state.characters):
                    page_name = self.state.characters[self.current_index].get("name")
                    if page_name:
                        self.page_cache.add_image_to_cache(self.current_book or "default", page_name, image_path)
                
                # 캐시 업데이트
                if hasattr(self, 'page_cache') and 0 <= self.current_index < len(self.state.characters):
                    page_name = self.state.characters[self.current_index].get("name")
                    if page_name:
                        self.page_cache.remove_image_from_cache(self.current_book or "default", page_name, image_path)
                
            if self.current_book and self.current_book in self.state.books:
                self.state.books[self.current_book]["pages"] = self.state.characters
                self.save_to_file()
    
    def on_thumbnail_image_removed(self, image_path):
        """썸네일바에서 이미지 제거 시 페이지 데이터 업데이트"""
        if 0 <= self.current_index < len(self.state.characters):
            char = self.state.characters[self.current_index]
            
            # additional_images에서 제거
            if "additional_images" in char and image_path in char["additional_images"]:
                char["additional_images"].remove(image_path)
                print(f"[DEBUG] 페이지 데이터에서 이미지 제거: {os.path.basename(image_path)}")
                
                # 메인 이미지 경로도 확인해서 제거
                if char.get("image_path") == image_path:
                    char["image_path"] = ""
                    print(f"[DEBUG] 메인 이미지 경로도 제거됨")
    ...
    def on_thumbnail_images_updated(self, image_list):
        """썸네일바 이미지 목록 전체 업데이트"""
        if 0 <= self.current_index < len(self.state.characters):
            char = self.state.characters[self.current_index]
            
            # 첫 번째 이미지를 메인 이미지로, 나머지를 additional_images로 설정
            if image_list:
                char["image_path"] = image_list[0]
                char["additional_images"] = image_list[1:] if len(image_list) > 1 else []
            else:
                char["image_path"] = ""
                char["additional_images"] = []
            
            print(f"[DEBUG] 페이지 이미지 목록 전체 업데이트: {len(image_list)}개")
            
            # 캐시 업데이트
            if hasattr(self, 'page_cache') and 0 <= self.current_index < len(self.state.characters):
                page_name = self.state.characters[self.current_index].get("name")
                if page_name:
                    self.page_cache.put(self.current_book or "default", page_name, image_list)
    ...
    def restore_book_list(self):
        ...
        try:
            ...
            # 백업 복구 전 기존 이미지 정리
            print("[DEBUG] 백업 복구 - 기존 이미지 정리 시작")
            cleanup_all_images_on_backup_restore()
            print("[DEBUG] 기존 이미지 정리 완료")
