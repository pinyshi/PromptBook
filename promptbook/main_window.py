...
class MainWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        # 위젯들을 레이아웃에 배치
        self.arrange_ui_components()
    
    def arrange_ui_components(self):
        """UI 컴포넌트들을 레이아웃에 배치합니다."""
        # 왼쪽 패널: 북 리스트
        book_label = QLabel("📚 북 리스트")
        book_label.setStyleSheet("font-weight: bold; font-size: 14px; padding: 5px;")
        self.left_layout.addWidget(book_label)
        ...
        # 캐릭터 리스트
        char_label = QLabel("👥 캐릭터 리스트")
        char_label.setStyleSheet("font-weight: bold; font-size: 14px; padding: 5px;")
        self.left_layout.addWidget(char_label)
        self.left_layout.addWidget(self.sort_selector)
        self.left_layout.addWidget(self.character_list)
        ...
        self.duplicate_button.clicked.connect(self.duplicate_selected_character)
    ...
    def restore_deleted_page(self, page_data):
        """삭제된 페이지를 복구합니다."""
        if not self.current_book:
            return
            
        # 복구할 페이지 데이터에서 삭제 시간 정보 제거
        page_data = page_data.copy()
        page_data.pop("deleted_time", None)
        ...
        # 페이지 추가
        self.state.characters.append(page_data)
        ...
        # 삭제된 페이지 목록에서 제거
        self.deleted_pages[self.current_book] = [
            p for p in self.deleted_pages[self.current_book]
            if p.get("deleted_time") != page_data.get("deleted_time")
        ]
        self.save_deleted_pages()
        
        # UI 업데이트
        self.refresh_character_list(selected_name=page_data["name"])
        self.save_to_file()
        
        # 복구 완료 메시지
        QToolTip.showText(
            self.character_list.mapToGlobal(self.character_list.rect().center()),
            "페이지가 복구되었습니다."
        )
    ...
    def add_book(self):
        """새 북을 추가합니다."""
        try:
            name, ok = QInputDialog.getText(self, "새 북 추가", "북 이름을 입력하세요:")
            if ok and name.strip():
                name = name.strip()
                if name not in self.state.books:
                    self.state.books[name] = {
                        "emoji": "📕",
                        "pages": [],
                        "sort_mode": "커스텀 정렬"
                    }
                    ...
                    QMessageBox.warning(self, "중복된 이름", "이미 존재하는 북 이름입니다.")
        except Exception as e:
            print(f"북 추가 실패: {e}")
    ...
    def add_character(self):
        ...
        try:
            if not self.current_book:
                QMessageBox.warning(self, "북 선택 필요", "먼저 북을 선택해주세요.")
                return
            ...
            self.state.characters.append(new_character)
            self.state.books[self.current_book]["pages"] = self.state.characters
            
            # UI 업데이트
            item = QListWidgetItem("새 캐릭터")
            self.character_list.addItem(item)
            self.character_list.setCurrentItem(item)
            
            # 새 캐릭터 선택
            self.current_index = len(self.state.characters) - 1
            self.load_character_data(new_character)
    ...
    def copy_prompt_to_clipboard(self):
        ...
        try:
            ...
            clipboard.setText(self.prompt_input.toPlainText())
            QToolTip.showText(
                self.copy_button.mapToGlobal(self.copy_button.rect().center()),
                "프롬프트가 클립보드에 복사되었습니다."
            )
        except Exception as e:
    ...
    def load_preview_image(self):
        ...
        try:
            ...
            if file_path and self.current_index >= 0:
...
class WindowResizer(QWidget):
    ...
    def initUI(self):
        ...
        if self.position in ['top', 'bottom']:
            ...
            self.setCursor(Qt.SizeHorCursor)
        elif self.position in ['topleft', 'bottomright']:
            self.setCursor(Qt.SizeFDiagCursor)
    ...
    def mousePressEvent(self, event):
        if event.button() == Qt.LeftButton:
            ...
            self.windowPos = self.parent().pos()
            self.windowSize = self.parent().size()
            event.accept()
            
    def mouseMoveEvent(self, event):
        if event.buttons() == Qt.LeftButton and hasattr(self, 'mousePos'):
            delta = event.globalPos() - self.mousePos
            window = self.parent()
            
            if 'right' in self.position:
                width = self.windowSize.width() + delta.x()
                window.resize(max(window.minimumWidth(), width), window.height())
            elif 'left' in self.position:
                width = self.windowSize.width() - delta.x()
                if width >= window.minimumWidth():
                    window.resize(width, window.height())
                    window.move(self.windowPos.x() + delta.x(), window.y())
                    
            if 'bottom' in self.position:
                height = self.windowSize.height() + delta.y()
                window.resize(window.width(), max(window.minimumHeight(), height))
            elif 'top' in self.position:
                height = self.windowSize.height() - delta.y()
                if height >= window.minimumHeight():
                    window.resize(window.width(), height)
                    window.move(window.x(), self.windowPos.y() + delta.y())
